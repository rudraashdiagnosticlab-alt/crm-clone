      ...
      {tab === 'API & Keys' && (
        <div className="max-w-[760px] rounded-2xl border bg-card shadow-sm">
          ...
          <div className="p-[18px]">
            {cfg && [['Database', cfg.database.provider, cfg.database.configured], ['Redis', 'Cache / queues', cfg.redis.configured], ['Calling Provider', cfg.calling.provider, cfg.calling.configured], ['AI Provider', `${cfg.ai.provider}${cfg.ai.model ? ` · ${cfg.ai.model}` : ''}`, cfg.ai.configured], ['Quo Integration', cfg.quo.sandbox ? 'Sandbox mode' : 'Live', cfg.quo.configured], ['Object Storage', cfg.storage.provider, cfg.storage.configured]].map((k) => (
